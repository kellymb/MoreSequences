...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  2 red dots.'
    window = rg.RoseWindow(400, 400, title)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(centers)):
        point = centers[k]
        orb = rg.Circle(centers[k], 10)
        orb.fill_color = color
        orb.attach_to(window)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 1:  5 slices, thin (thickness=3) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(75, 75), 60)
    circle.outline_thickness = 1
    pizza(window, circle, 100, 'blue', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for p in range(len(points)):
        line = rg.Line(points[p], circle.center)
        line.color = color
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    window = rg.RoseWindow(100, 100, title)
    circle = rg.Circle(rg.Point(50, 50), 50)
    circle.outline_thickness = 1
    polygon(window, circle, 3, 'yellow3', 20)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    orb = generate_points_on_circle(circle, number_of_segments)
    for j in range(len(orb)):
        line = rg.Line(orb[j-1], orb[j])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(237, 163), 96)
    circle.fill_color = 'blue'
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for l in range(number_of_lines):
        ...
        line = rg.Line(points[l], points[(l+hops_to_next_point)%number_of_lines])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
        line.attach_to(window)
